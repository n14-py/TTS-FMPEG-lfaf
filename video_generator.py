...
def download_image_robust(url, save_path, retries=3):
    ...
    # Cabeceras (Headers) para engañar al servidor y parecer un humano en Chrome
    user_agent = "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/122.0.0.0 Safari/537.36"
    accept_header = "image/avif,image/webp,image/apng,image/svg+xml,image/*,*/*;q=0.8"
    
    for attempt in range(retries):
        try:
            # INTENTO 1: Usamos CURL añadiendo los Headers (-H)
            cmd = [
                "curl", "-L", "-k", "--retry", "2", 
            ...
            # Verificamos que la imagen realmente pesa algo (no está vacía o es un HTML de error)
            if os.path.exists(save_path) and os.path.getsize(save_path) > 1000: # Subimos a 1KB para descartar errores
                return True
                
        except Exception as e:
            logger.warning(f"⚠️ Intento {attempt+1} fallido con curl. Usando método alternativo.")
            try:
                # INTENTO 2: Fallback usando Python Requests con Headers
                time.sleep(2) # Pausa un poco más larga para no saturar
                headers = {
                    "User-Agent": user_agent,
                    "Accept": accept_header,
                    "Accept-Language": "es-ES,es;q=0.9",
                    "Referer": "https://www.google.com/" # Simulamos venir desde Google
                }
                r = requests.get(url, headers=headers, verify=False, timeout=15)
                
                # Verificamos que sea un código 200 y que sea realmente una imagen
                if r.status_code == 200 and 'image' in r.headers.get('Content-Type', ''):
                    with open(save_path, 'wb') as f: 
                        f.write(r.content)
                    return True
                else:
                    logger.warning(f"⚠️ El servidor respondió con código {r.status_code} o no es una imagen.")
